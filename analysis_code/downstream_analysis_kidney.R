...
sn_kidney_filtered <- readRDS(rds_sn_path_kidney)
ST_kidney <- readRDS(rds_ST_path_kidney)
...
Healthy <- Cellchat_pipeline(sn_kidney_filtered, "Tissue.Type", "Healthy", "cluster_annotation")
AKI  <- Cellchat_pipeline(sn_kidney_filtered, "Tissue.Type", "AKI", "cluster_annotation")
CKD  <- Cellchat_pipeline(sn_kidney_filtered, "Tissue.Type", "CKD", "cluster_annotation")

cellchat_whole_list <- list(Healthy = Healthy, AKI = AKI, CKD = CKD)
cellchat_sn_kidney <- mergeCellChat(cellchat_whole_list, add.names = names(cellchat_whole_list))

#obtain L-R matrix
L_R_matrix <- c()
for (state in c("Healthy", "AKI", "CKD")) {
...
library(ISCHIA)
df_kidney <- read.csv(path_abundance_matrix, row.names = 1)
wss_kidney <- Composition.cluster.k(df_kidney, 10)

ST_CC <- Composition.cluster(ST_kidney, df, 4)
...
H_AKI <- L_R_DE(cellchat_whole_list, "Healthy", "AKI")
H_CKD <- L_R_DE(cellchat_whole_list, "Healthy", "CKD")
AKI_CKD<- L_R_DE(cellchat_whole_list, "AKI", "CKD")
